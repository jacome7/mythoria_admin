...
export default function AIUsagePage() {
  ...
  const getActionInfo = (action: string) => {
    const actionMap: Record<string, { icon: string; name: string; description: string }> = {
      story_structure: {
        ...
        description: 'Creating the story framework and plot',
      },
      story_outline: {
        icon: '📋',
        name: 'Story Outline',
        description: 'Generating story outlines and summaries',
      },
      chapter_writing: {
        icon: '✍️',
        name: 'Chapter Writing',
        description: 'Writing story chapters and content',
      },
      image_generation: {
        icon: '🎨',
        name: 'Image Generation',
        description: 'Creating story illustrations and images',
      },
      story_review: {
        icon: '🔍',
        name: 'Story Review',
        description: 'Reviewing and analyzing story content',
      },
      character_generation: {
        icon: '👤',
        name: 'Character Generation',
        description: 'Creating characters and personas',
      },
      story_enhancement: {
        icon: '✨',
        name: 'Story Enhancement',
        description: 'Improving and refining story content',
      },
      audio_generation: {
        icon: '🎵',
        name: 'Audio Generation',
        description: 'Creating audio content and narration',
      },
      content_validation: {
        icon: '✅',
        name: 'Content Validation',
        description: 'Validating content quality and safety',
      },
      image_edit: {
        icon: '🖼️',
        name: 'Image Editing',
        description: 'Editing and modifying images',
      },
      test: { icon: '🧪', name: 'Test', description: 'Testing and experimental features' },
    };
    return (
      actionMap[action] || { icon: '📝', name: action.replace(/_/g, ' '), description: 'AI action' }
    );
  };
  ...
  const fetchAIUsageData = useCallback(async () => {
    try {
      ...
      const transformedProviders: AIProvider[] = data.modelBreakdown.map(
        (
          model: {
            model: string;
            totalTokens: number;
          ...
          dailyUsage: Math.round(
            model.totalTokens /
              (timePeriod === '1d' ? 1 : timePeriod === '7d' ? 7 : timePeriod === '30d' ? 30 : 90),
          ),
      ...
      const transformedActionUsage: ActionUsage[] = data.actionBreakdown.map(
        (action: {
          action: string;
          totalCost: number;
          requests: number;
        ...
        {activeTab === 'providers' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {providers.map((provider) => (
              <div
                key={provider.id}
                className={`card bg-base-100 shadow-xl ${!provider.isActive ? 'opacity-60' : ''}`}
              >
                <div className="card-body">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-lg font-bold">
                        {provider.type === 'text' &&
                          `${(provider.dailyUsage / 1000).toFixed(0)}K tokens`}
                        {provider.type === 'image' && `${provider.dailyUsage} images`}
                        {provider.type === 'audio' &&
                          `${(provider.dailyUsage / 60).toFixed(0)} minutes`}
                      </span>
                    ...
                    <tbody>
                          ...
                          <tr key={usage.action}>
                            ...
                            <td>
                              <div className="font-semibold">
                                {usage.uniqueStories.toLocaleString()}
                              </div>
                            ...
                            <td>
                              <div className="font-bold text-primary text-lg">
                                €{usage.averageCostPerStory.toFixed(3)}
                              </div>
                      ...
                      {actionUsage.length > 0 && (
                        <tr className="border-t-2 border-base-300 bg-base-200/50">
                          ...
                          <td className="font-bold">
                            {(
                              actionUsage.reduce((sum, usage) => sum + usage.inputTokens, 0) /
                              1000000
                            ).toFixed(2)}
                            M
                          </td>
                          <td className="font-bold">
                            {(
                              actionUsage.reduce((sum, usage) => sum + usage.outputTokens, 0) /
                              1000000
                            ).toFixed(2)}
                            M
                          </td>
                          ...
                          <td className="font-bold">
                            {actionUsage
                              .reduce((sum, usage) => sum + usage.uniqueStories, 0)
                              .toLocaleString()}
                          </td>
                          ...
                          <td className="font-bold">
                            ...
                            {(
                              ...
                              Math.max(
                                actionUsage.reduce((sum, usage) => sum + usage.requests, 0),
                                1,
                              )
                            ).toFixed(4)}
                          </td>
                          <td className="font-bold text-secondary text-lg">
                            ...
                            {(
                              actionUsage.reduce((sum, usage) => sum + usage.totalCost, 0) /
                              Math.max(
                                actionUsage.reduce((sum, usage) => sum + usage.uniqueStories, 0),
                                1,
                              )
                            ).toFixed(3)}
                          </td>
