...
export async function GET(request: Request) {
  try {
    ...
    const sortBy = (searchParams.get('sortBy') || 'serviceCode') as 'serviceCode' | 'credits' | 'isActive' | 'createdAt';
    const sortOrder = (searchParams.get('sortOrder') || 'asc') as 'asc' | 'desc';

    // Get services data
    const services = await adminService.getPricingServices(page, limit, searchTerm, sortBy, sortOrder);
