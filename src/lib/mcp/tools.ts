...
export function registerMcpTools(server: McpServer) {
  ...
  server.tool(
    ...
    "Get main project statistics and KPIs like total users, stories, and open tickets.",
    {},
    async () => {
      try {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ users, stories, openTickets: ticketMetrics.openTickets + ticketMetrics.inProgressTickets }) }] };
      } catch (e: unknown) { return { isError: true, content: [{ type: "text", text: `Error: ${toolErr(e)}` }] }; }
    }
  );
  ...
  server.tool(
    ...
    async ({ authorId, amount, eventType }) => {
      try {
        ...
        return { content: [{ type: "text", text: `Successfully assigned ${amount} credits to ${authorId}.` }] };
      } catch (e: unknown) { return { isError: true, content: [{ type: "text", text: `Error: ${toolErr(e)}` }] }; }
    }
  );
  ...
  server.tool(
    ...
    async ({ storyId }) => {
      try {
        ...
        return { content: [{ type: "text", text: JSON.stringify({ message: "Restart triggered successfully.", run }) }] };
      } catch (e: unknown) { return { isError: true, content: [{ type: "text", text: `Error: ${toolErr(e)}` }] }; }
    }
  );
  ...
  server.tool(
    ...
    async ({ ticketId, status }) => {
      try {
        ...
        return { content: [{ type: "text", text: `Ticket ${ticketId} status updated to ${status}.` }] };
      } catch (e: unknown) { return { isError: true, content: [{ type: "text", text: `Error: ${toolErr(e)}` }] }; }
    }
  );
  ...
  server.tool(
    ...
    async ({ id, payload }) => {
      try {
        const { adminBlogService } = await import("@/db/services");
        const post = await adminBlogService.upsertTranslation(id, payload as unknown as AdminTranslationInput);
        return { content: [{ type: "text", text: JSON.stringify(post) }] };
      } catch (e: unknown) { return { isError: true, content: [{ type: "text", text: `Error: ${toolErr(e)}` }] }; }
    }
  );
  ...
  server.tool(
    ...
    async ({ id, action }) => {
      try {
        const { adminBlogService } = await import("@/db/services");
        const result = action === "publish" ? await adminBlogService.publish(id) : await adminBlogService.archive(id);
  ...
  server.tool(
    ...
    async ({ payload, adminEmail }) => {
      try {
        const { campaignService } = await import("@/db/services/campaigns");
        const created = await campaignService.createCampaign(payload as CreateCampaignInput, adminEmail);
  ...
  server.tool(
    ...
    async ({ id, payload, adminEmail }) => {
      try {
        const { campaignService } = await import("@/db/services/campaigns");
        const updated = await campaignService.updateCampaign(id, payload as UpdateCampaignInput, adminEmail);
  ...
  server.tool(
    ...
    async ({ id, adminEmail }) => {
      try {
        const { campaignService } = await import("@/db/services/campaigns");
        const transitioned = await campaignService.transitionCampaignStatus(id, "paused", adminEmail);
