// Synced blog schema from mythoria-webapp
...
export const blogPostTranslations = pgTable(
  ...
  (table) => ({
    postLocaleUnique: uniqueIndex('blog_post_translations_post_id_locale_unique').on(
      table.postId,
      table.locale,
    ),
    slugUnique: uniqueIndex('blog_post_translations_slug_unique').on(table.slug),
  }),
);
