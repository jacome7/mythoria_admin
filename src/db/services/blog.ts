...
export const adminBlogService = {
  async list(
    ...
    if (localeFilter)
      list = list.filter((p) => p.translations.some((t) => t.locale === localeFilter));
    if (search)
      list = list.filter(
        (p) =>
          p.slugBase.includes(search) ||
          p.translations.some((t) => t.title?.toLowerCase().includes(search)),
      );
    return list;
  },
  ...
  async update(id: string, input: AdminUpdatePostInput) {
    ...
    if (input.publishedAt !== undefined) {
      ...
      postUpdate.publishedAt = (v
        ? new Date(v as string | Date)
        : null) as unknown as typeof blogPosts.$inferInsert.publishedAt;
    }
