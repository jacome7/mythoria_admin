...
| Promotions    | `GET /api/admin/promotion-codes`                    | Paginated promotion-code inventory with `search`, `type`, and `active` filters.                                                                                                  |
| Promotions    | `POST /api/admin/promotion-codes`                   | Create a promotion code. Normalizes `code` to uppercase and validates credit and redemption limits.                                                                              |
| Promotions    | `GET /api/admin/promotion-codes/[id]`               | Fetch promotion-code detail plus redemption aggregates.                                                                                                                          |
| Promotions    | `PUT /api/admin/promotion-codes/[id]`               | Edit any create-time field only while the code has zero redemptions. Returns `409` once used or when the edited code conflicts.                                                  |
...
| Fiscal Docs   | `POST /api/admin/fiscal-documents/[id]/mark-issued` | Marks an eligible fiscal document as locally issued after manual KeyInvoice/accounting verification. Requires document identity, issued date, reason, and explicit confirmation. |
...
| Stories       | `GET /api/admin/stories`                            | Story moderation feed. Query params: `page`, `limit`, `search`, `status`, `featured`, `target_audience`, `novel_style`, `graphical_style`, `sortBy`, `sortOrder`.                |
